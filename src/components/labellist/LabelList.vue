<template>
  <div class="label-list-box">
    <!-- 循环生成标签列表 -->
    <div :draggable="editMode" @dragend="dragendFn()" @dragstart="handleDragstart(index)" @drop.prevent="handleDrop()"
         @dragover.prevent="handleDragover(index)" v-for='(item,index) in labelList' :key='index' class="label"
         @click.prevent='changeLabel(index ,"click")' @mouseenter="changeLabel(index,'hover')"
         @mouseleave="changeLabel(-1,'hover')"
         :class='{currentClick:currentId===index,currentHover:currentHoverLabel===index,isDragging:index === newItemIndex}'>
      <!-- 标签标题 -->
      <i class="iconfont">&#xe610;</i>
  ...
  <!-- 添加单个标签的弹窗 -->
  <Dialog :dialog-visible='isShowLabelDialog'
          @cancle='closeLabelDialog'
          @confirm="handleLabel(handleType)"
          :title='dialogTitle'>
    <wh-form ref='form'>
      <wh-input :rules='rules'
                type="text"
                v-model="labelTitle"
                placeholder="请输入标签名称"
                padding='50px 30px'
                maxlength='10'></wh-input>
    </wh-form>
  </Dialog>
</template>
...
import {LabelListProps} from '@/components/bookmark/index';
import {HandleLabelType, LabelState} from '@/components/labellist/index';
import WhForm from '@/baseComponents/form/Wh-form.vue';
import Dialog from '@/baseComponents/dialog/Dialog.vue';
import useDialog from '@/components/labellist/useDialog';
...
export default defineComponent({
  ...
  setup(props, {emit}) {
    // 获取form元素
    const form = ref<InstanceType<typeof WhForm>>(null);
